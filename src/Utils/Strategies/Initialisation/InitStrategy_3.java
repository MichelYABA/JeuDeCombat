...
/*
 Troisieme  strategie d'intialisation par defaut
A la difference de la strategie par defaut, elle est caracterisee par la presence des murs uniquements 
en haut et en bas de la grille
*/
public class InitStrategy_3 implements InitStrategy{

    private int[][] g;
    private int nc;
    private int[] tab;
     //Positions possibles des murs dans la grille
    private final int[][] posMur ={{2, 3}, {3,3}, {5, 2}, {6,2}, {2,11}, {2,12}, {7,13}, {7,14}, {10,4}, {10,5}, {13,12}, {12,12}};
     ...
     * @param grille
     */
    public InitStrategy_3(int[][] grille){
        this.g = grille;
    }
    ...
    public void initGrille(int n) {
        ...
        for (int[] p : posPastille) {
         ...
         /* Initialisation des combattants a l'interieur de la grille*/
        ...
        /*
        Inititialisation des murs en haut en bas de la grille
        */
        
        //on place les murs sur chaque colonne de la premiere ligne
        for(int a=0; a<(this.g[0].length); a++){
            g[0][a] = Definitions.MUR;
        }
                
        // on place les murs sur chaque colonnes de la derniere ligne
        for(int j=0; j<this.g[0].length; j++ ){
            g[tailleL-1][j] = Definitions.MUR;
        }
